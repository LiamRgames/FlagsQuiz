...
class Results:
    ...
    def display_stats(self, wrong,right,total_questions_answered):
        ...
        percent = (right/total_questions_answered) * 100
        percentage_correct = Label(stats_frame,font=('Sans Serif', 10, 'bold'), text=f"Questions Correct Percentage: {percent:.2f}%")
        if percent > 90:
            emoji_image ='emoji/cool.png'
            percentage_correct_image = ImageTk.PhotoImage(Image.open(emoji_image).resize((300, 300)))
            percentage_correct.image = percentage_correct_image
            percentage_correct.config(image=f'{percentage_correct_image}', bg='lightblue',compound='bottom')
        elif 90 >= percent >= 50:
            emoji_image = 'emoji/thumbs_up.png'
            percentage_correct_image = ImageTk.PhotoImage(Image.open(emoji_image).resize((300, 300)))
            percentage_correct.image = percentage_correct_image
            percentage_correct.config(image=f'{percentage_correct_image}', bg='lightblue',compound='bottom')
        elif 50 > percent:
            emoji_image = 'emoji/thumbs_down.png'
            percentage_correct_image = ImageTk.PhotoImage(Image.open(emoji_image).resize((300, 300)))
            percentage_correct.image = percentage_correct_image
            percentage_correct.config(image=f'{percentage_correct_image}', bg='lightblue',compound='bottom')
        ...
        frame_toggle(question_frame,stats_frame, [stats_heading,total_right,total_wrong,total_questions, percentage_correct,back_button])
...
while True:
    # Overlay for Screen Size Changing
